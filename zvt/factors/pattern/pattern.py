...
class ZenTransformer(Transformer):
    ...
    def transform_one(self, entity_id, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['duan_state'] = 'yi'
        # 段的切换点
        df['duan_up_point'] = False
        df['duan_down_point'] = False
                    ...
                    if opposite_count >= 4 or (index - can_fenxing_index >= 8):
                        ...
                        if fenxing_list != None:
                            ...
                            if len(fenxing_list) == 4:
                                current_duan_state = handle_duan(fenxing_list=fenxing_list,
                                                                 pre_duan_state=current_duan_state)
                                if current_duan_state != 'yi':
                                    # 确定状态
                                    df.loc[fenxing_list[0].index:fenxing_list[-1].index,
                                    'duan_state'] = current_duan_state

                                    if current_duan_state == 'up':
                                        df.loc[fenxing_list[0].index, 'duan_up_point'] = True
                                    else:
                                        df.loc[fenxing_list[0].index, 'duan_down_point'] = True
                                    # 只留最后一个
...
if __name__ == '__main__':
    ...
    # 处理段
    up = one_df[one_df.duan_up_point][['timestamp', 'high']]
    down = one_df[one_df.duan_down_point][['timestamp', 'low']]
    df1 = up.rename(columns={"high": "value"})
    df2 = down.rename(columns={"low": "value"})
